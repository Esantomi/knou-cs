...
- **동적 해싱(Dynamic Hashing)**
  ...
  - 데이터베이스의 증대 혹은 축소에 따른 인덱스의 구조를 조절하기 위해 해시 함수를 동적 변경하는 기술
    - 해시 함수는 디렉터리라는 구조로 표현됨
    - 즉, 동적 해시는 디렉터리를 통해 버킷에 접근할 수 있는 2단계 구조를 지님
- **확장성 해싱(Extendible Hashing)**
  - 동적 해싱의 일종으로 디렉터리와 버킷의 2단계 구조
    - **디렉터리(Directory)** : 디스크에 저장되는 버킷 주소 테이블
    - 디렉터리 깊이를 의미하는 정수값 𝑑를 포함하는 헤더와 데이터가 저장된 버킷에 대한 2<sup>𝑑</sup>개의 포인터로 구성
  - **모조키(pseudo key)**
    - 레코드의 탐색키 값이 해시 함수에 의해 일정 길이의 비트 스트링으로 변환된 키
      - 탐색키를 해시 함수로 변환한 가짜 키
      - 탐색키를 단순히 다른 식의 바이트 표현으로 전환한 것이고, 버킷ID를 의미하는 것은 아님
    - 모조키의 첫 𝑑 비트를 사용하여 디렉터리에 접근
      - 버킷 헤더를 통해 앞부분 𝑑 비트의 개수를 나타냄
  - **버킷 헤더(bucket header)**
    - 정수값 𝑖(≤𝑑)가 저장되어 있음을 표시
    - 𝑖는 버킷에 저장되어 있는 레코드의 모조키들이 처음부터 𝑖비트까지 일치함을 표시
  - 확장성 해싱의 구조  
    ![image](https://user-images.githubusercontent.com/61646760/154228903-d7aa6122-ee37-43b5-a2e5-bb2ce2bb6e16.png)
    - 디렉터리 헤더의 3은 모조키 앞 3비트를 보겠다는 의미
      - 버킷 헤더 3도 모조키 앞 3비트를 보겠다는 의미 (B<sub>3</sub>의 2는 2비트만 보겠다는 것)
    - 디렉터리는 000으로 시작하면 모두 B<sub>1</sub>에 배치
      - 01로 시작하면 B<sub>3</sub>에, 1로 시작하면 B<sub>4</sub>에 배치
    - `예) 모조키가 101000010001이라면?`
      - 해시 함수 ℎ(𝐾)를 통해 모조키 101000010001을 도출했다면, 모조키의 앞 3비트, 버킷 헤더가 101이므로 B<sub>4</sub>에 있음
      - 즉, 확장성 해싱은 해시 함수를 통해 곧바로 버킷ID에 도달하는 게 아니라, 해시 함수를 통해 디렉터리로 갔다가 버킷으로 도달
    - `예) 모조키 101000010001로 새로 저장하고 싶다면?`
      - B<sub>4</sub>에 인덱스 엔트리를 저장할 공간이 없음 → 오버플로우
        - 정적 해싱은 버킷 추가가 불가하므로, 다음 버킷이나 연쇄 버킷에 저장함
        - 동적 해싱은 버킷 개수 추가가 가능함
      - 확장성 해싱의 분할  
        ![image](https://user-images.githubusercontent.com/61646760/154230679-af9ba946-c2d8-4d0e-9c93-ab84a42e4e73.png)
        - 모조키 시작 비트 스트링 1을 10과 11로 분할하여 버킷을 하나 추가함
          - 레코드 삽입에 의해 분할된 확장성 해싱 파일
        - 버킷 하나에 대한 재분배는 성능 저하에 큰 영향이 없으므로 자주 사용되는 해싱 기법
### 비트맵 인덱스
- **비트맵 인덱스(Bitmap Index)**
  - 탐색키의 중복 비율이 높은 컬럼을 대상으로 하는 질의를 효율적으로 처리하기 위해 고안된 특수한 형태의 인덱스
    - 탐색키가 너무 많이 중복된 상황에서는 해시 인덱스도 순차 인덱스도 효율적인 구조를 제공하지 못함
    - `예) 성별` (중복이 많으므로 오버플로우가 심각함)
- **비트맵(Bitmap)**
  - 간단한 비트의 배열
    - 릴레이션 𝑟의 속성 𝐴에 대한 비트맵 인덱스는 𝐴가 가질 수 있는 값에 대해 비트맵을 구성
    - 각 비트맵은 릴레이션에 있는 레코드의 수 𝑛개 만큼 𝑛개의 비트로 표현
  - 비트맵 인덱스의 구성
    - 𝑖번째 레코드가 컬럼 𝐴에 해당 값을 가지면 비트맵의 𝑖번째 비트를 `1`로 그렇지 않으면 `0`으로 설정
      ![image](https://user-images.githubusercontent.com/61646760/154234692-291c3d5f-bbd1-4e3b-8783-60ca40880b4f.png)
      - 성별에 대해 인덱스를 만든다면, 비트맵 인덱스(남자 비트열 + 여자 비트열)를 남자(`10`), 여자(`01`)로 만들 수 있음
      - 비트열을 보면 남녀 성별 비트맵을 확인할 수 있음
    - [`예) 성적 비트맵 인덱스`](https://user-images.githubusercontent.com/61646760/154238059-eb933f75-e0b2-417b-b120-c0bb302fa38d.png)
      - A, B, C, D, F의 5개의 비트열
      - 생성된 성적 비트맵은 A(001000), B(100100), C(010000), D(000010), F(000001)
  - 비트맵 인덱스의 사용
    - 성별이 남자이고 성적이 B 인 학생의 정보를 출력
      ```
      SELECT * FROM 학생
        WHERE 성별 = '남자' AND 성적 = 'B'
      ```
    - 성별의 '남자'와 성적의 'B'의 비트열에 대한 비트 논리곱 연산을 수행  
      ![image](https://user-images.githubusercontent.com/61646760/154240145-86f9e1e3-38c0-428d-9a81-a26ccb4a0127.png)
        - 비트맵 인덱스의 구성 부분에서 [성별 비트맵, 성적 비트맵](https://user-images.githubusercontent.com/61646760/154240028-c7a360e4-0dc4-40c0-ac92-b849f829a8d3.png) 구축 완료
        - 남자에 대한 비트열(100101), B에 대한 비트열(100100) 간의 AND(`&`) 연산 수행
          - 각각의 비트에 대한 AND 연산 수행
          - 즉, 1과 1이면 1, 0과 0이면 0, 1과 0이면 0
    - 비트 논리곱 연산 수행 결과  
      ![비트 논리곱 연산 수행 결과](https://user-images.githubusercontent.com/61646760/154241743-744ce5d4-bee1-4770-8f7f-10fdbc0053c7.png)
      - 연산 결과는 100100
      - 1번째, 4번째 레코드가 조건을 만족함을 의미
      - 해당 레코드가 저장된 블럭을 메모리에 적재함으로써 빠르게 결과 제공이 가능함
  - 비트맵 인덱스의 특징
    - 비트맵의 활용
      - 컬럼에 대한 값의 범위가 유한하고 비교적 개수가 적은 규모일 때 용이
        - 가짓수가 적고 중복의 비율이 매우 높은 컬럼에 대해 적용 (대략 10개 미만일 때)
        - 종류가 많은 기본키 등에 적용할 시 성능이 저하될 수 있음
      - `예) 직책, 학과, 혈액형 등`
    - 비트맵 인덱스의 크기
...
### 트랜잭션의 이해
- 데이터 동시 접근의 문제
  - 동일 데이터에 다수 사용자의 접근 허용 시 일관성이 훼손됨
  - [`예) 두 고객에 의해 입금과 출금이 동시에 이루어지는 경우 : 잔고가 충돌`](https://user-images.githubusercontent.com/61646760/154479988-8c98f078-2435-42c3-a423-35dba3a23e18.png)
- **트랜잭션(Transaction)**
  - 트랜잭션의 개념
    - 데이터베이스를 조작하기 위한 하나의 논리적 단위를 이루는 일련의 연산의 집합
      ...
      - 논리적 작업은 하나(예금 인출)지만, 데이터베이스 입장에서 연산은 총 3가지(Read, -, Write)가 이루어짐
    - 데이터베이스를 사용하여 처리하는 작업을 하나의 묶음으로 인식하여 **묶음 단위로 실행되도록 정의**한 개념
      - 그 묶음이 곧 트랜잭션
      - 무조건 트랜잭션 단위로 데이터를 처리
  - 트랜잭션의 특징
    ...
    - ACID 특성 유지
      - [`예) 원자성`](https://user-images.githubusercontent.com/61646760/154492249-9c8a0fda-2023-4aae-9d80-7c65861aa980.png)
        - 2행을 처리하던 도중 오류가 발생해 트랜잭션을 마무리할 수 없게 되면, 원상복구시킴
      - [`예) 일관성`](https://user-images.githubusercontent.com/61646760/154492658-4c8e61cc-5291-4a4d-bdd6-74db470b0d33.png)
        - A, B 계좌에 도합 20,000원이 있을 때, 2행에서 1,000원을 뺌으로써 일관성이 깨지지만, 5행에서 B 계좌에 1,000원을 넣어 줌으로써 일관성이 지켜짐
        - 트랜잭션이 실행된 순간 일관성이 훼손되지만, 전체 과정이 끝나면 일관성이 유지됨
      - [`예) 고립성`](https://user-images.githubusercontent.com/61646760/154493142-5099ac63-c282-4ba1-b23e-517c0dff2247.png)
        - 하나의 트랜잭션이 처리되고 있는 동안, 다른 트랜잭션은 처리될 수 없음
      - [`예) 지속성`](https://user-images.githubusercontent.com/61646760/154493346-cebf50a9-a50c-40db-aaaf-b2c9cdbd16f4.png)
        - 21,000은 오기 (11,000원임)
        - 트랜잭션이 모두 끝난 다음 문제가 발생해 데이터가 깨져도 원상복구시킴
  - 트랜잭션 연산자
    - 트랜잭션의 연산
    ...
    - 트랜잭션 실행의 연산
      - `Commit` : 트랜잭션 연산에 의해 갱신된 데이터 항목의 값을 데이터베이스에 반영시키고 지속성을 확보하는 연산
        - 즉, 트랜잭션이 모두 끝난 후, 데이터베이스 반영이 완료되고, 정상적으로 트랜잭션을 끝냈다는 완료 버튼을 누르는 기능
      - `Rollback` : 트랜잭션이 중단되기 이전까지 수행한 연산에 의해 갱신된 모든 데이터 항목의 값을 무효화하여 일관성을 확보하는 연산
        - 트랜잭션이 도중에 실행 중단되거나, 처리 결과의 데이터베이스 반영이 불가할 때, 원자성에 의해 실행 전의 상태로 원상복구시키는 기능
    - 트랜잭션의 5가지 상태 변화  
      ![image](https://user-images.githubusercontent.com/61646760/154494874-7460e60c-950f-4f33-91e9-abfb4236155c.png)
      - 동작 : 트랜잭션 시작을 준비 또는 실행 중인 상태
      - 부분 커밋 : 마지막 연산을 실행한 직후의 상태 (데이터베이스 미반영, 트랜잭션 메모리에만 적재된 상태)
      - 커밋 : 모든 실행이 성공적으로 완료된 후의 상태 (실제 데이터베이스 반영 완료)
      - 실패 : 실행이 정상적으로 진행될 수 없는 상태 (트랜잭션 도중 오류가 발생한 상태)
...
- 동시성 고려
  - DBMS는 다수의 사용자가 데이터베이스를 공용으로 사용하기 위한 목적으로 도입
    ...
    - 하지만 서로 다른 데이터에 대한 트랜잭션이라면, 동시에 처리돼도 아무 문제가 없음
  - 트랜잭션 동시 실행의 이점
    - 트랜잭션 처리율과 자원 이용률을 향상
      - 일관성이 훼손되지 않는 선에서 여러 작업을 동시에 처리할 수 있으면 작업 효율이 증진됨
    - 트랜잭션의 대기 시간을 감소
      - 사용자의 만족도, 신뢰도를 향상시킬 수 있음
  - **동시성 제어(concurrency)**
    - 다수의 트랜잭션이 성공적으로 동시에 실행되어도 일관성을 유지할 수 있도록 지원하는 기법
- **스케줄(schedule)**
  - 다수의 트랜잭션에 포함된 연산의 실행 순서를 명시한 것
    - 스케줄을 확인해 동시 실행 여부 판별
- **직렬 스케줄(Serial Schedule)**
  - 𝑇<sub>0</sub>과 𝑇<sub>1</sub>이 순차적으로 실행되는 스케줄
  - 즉, 하나의 트랜잭션이 실행되면 해당 트랜잭션이 완료되어야 다른 트랜잭션이 실행될 수 있음
- **병렬 스케줄(Nonserial Schedule)**
  - 𝑇<sub>0</sub>과 𝑇<sub>1</sub>이 비순차적으로 실행되는 스케줄
  - 즉, 하나의 트랜잭션이 완료되기 전에 다른 트랜잭션이 실행되는 스케줄
    - 병렬 스케줄의 순서로 연산을 수행할 경우 일관성 훼손 발생 가능
  - [`예) 두 트랜잭션에 포함된 연산이 번갈아 처리되었지만 일관성이 유지되는 경우`](https://user-images.githubusercontent.com/61646760/154717968-e1676376-ab76-4b84-a485-00c2a8300191.png)
    - 스케줄3은 일관성 유지, 스케줄4는 A값(9,000원), B값(22,000원)의 합이 31,000원이 되므로 일관성이 훼손됨
- **직렬 가능 스케줄(Serializable Schedule)**
  - 트랜잭션 간 연산 순서를 교환하여 트랜잭션을 직렬 스케줄과 동등하게 변환이 가능한 스케쥴
    - 복수 개의 트랜잭션이 동시에 수행된 결과가 직렬 스케줄의 결과와 동일한 스케줄
  - 사용된 Read와 Write 연산 교환 시 상황에 따라 실행 결과의 일관성이 훼손되는 현상(충돌)이 발생
    - **충돌(Conflict)** : 연산의 순서를 바꿨을 때, 일관성이 훼손되는 교환
  - 연산 순서의 교환 (단, 𝐼<sub>𝑖</sub>는 𝑇<sub>𝑖</sub>의 연산)
    - 𝐼<sub>𝑖</sub>=Read(Q)   𝐼<sub>𝑗</sub>=Read(Q)
      - 충돌이 발생하지 않는 유일한 경우 (나머지 셋은 충돌 발생)
      - Write이 하나라도 있으면 절대 순서를 교환하면 안 됨
    - 𝐼<sub>𝑖</sub>=Read(Q)   𝐼<sub>𝑗</sub>=Write(Q)
    - 𝐼<sub>𝑖</sub>=Write(Q)  𝐼<sub>𝑗</sub>=Read(Q)
    - 𝐼<sub>𝑖</sub>=Write(Q)  𝐼<sub>𝑗</sub>=Write(Q)
  - **충돌 동등(Conflict Equivalent)**
    - 특정 스케줄 𝑆에서 충돌이 일어나지 않는 연산의 순서를 바꿔 스케줄 𝑆'으로 변환이 가능한 상태
      - [`예) 교환 불가능한 연산`](https://user-images.githubusercontent.com/61646760/154722377-926e7f50-61ac-439b-9252-fe2a3d82cad3.png)
        - Write이 있을 경우, 순서를 교환하면 충돌이 발생
      - [`예) 교환 가능한 두 쌍의 연산`](https://user-images.githubusercontent.com/61646760/154722853-e5d3604b-f47f-4a93-8d1f-3b33e1502ad1.png)
        - Write이 있어도, 서로 다른 데이터에 대한 연산이므로 교환해도 충돌이 발생하지 않음
        - 연산의 순서를 교환해도 결과가 교환 이전과 동일하므로 충돌 동등임
  - **충돌 직렬성(Conflict Serializability)**
    - 순서 교환이 가능한 연산을 교환하여 직렬 스케줄의 연산과 동등하게 변환이 가능한 스케줄  
      ![image](https://user-images.githubusercontent.com/61646760/154723462-77d0d9ba-57c7-4c5b-a7b0-737a308d2ac4.png)
    - 충돌 직렬성을 가진 스케줄은 일관성 훼손이 발생하지 않음
### 트랜잭션의 회복
- **회복(Recovery)**
...
- 회복 가능한 스케줄
  ...
  - 연쇄적 롤백(Cascade Rollback) 유발이 가능하다는 문제점이 있음  
    ...
    - Abort는 실패
    - 𝑇<sub>7</sub>의 롤백으로 인하여 연쇄적으로 다른 트랜잭션도 롤백
    - 하나의 실패로 인해 다수의 트랜잭션에서 모두 회복 연산을 수행해야 함
- **비연쇄적 스케줄(Cascadeless Schedules)**  
  ![image](https://user-images.githubusercontent.com/61646760/154731662-3644e08a-28ed-403e-81fd-8df1ea2a61a9.png)
  - 연쇄적 롤백으로 발생할 수 있는 대량의 회복 연산을 방지하기 위해 연쇄적이지 않은 스케줄로 구성된 스케줄
  - 𝑇<sub>𝑖</sub>가 기록한 데이터를 읽을 때 𝑇<sub>𝑖</sub>의 커밋이 𝑇<sub>𝑗</sub>의 읽기 연산보다 먼저 나타나는 스케줄
    - 회복 가능하면서도 연쇄적 롤백을 일으키지 않음
