...
- 강화학습의 적용 사례
  - Google DeepMind, AlphaGo
    - 프로기사를 맞바둑으로 이긴 최초의 프로그램 (2016.3)
    - 몬테카를로 트리 탐색, 딥러닝, 강화학습 기술의 결합
    - 이세돌과의 대국 이후 개량된 버전 출시
      - 2017.10. ‘AlphaGo Zero’ : 바둑 규칙만 입력, 스스로의 대국을 통해 이치 터득
      - 2017.12. ‘AlphaZero’ : 모든 보드게임을 위한 알파고 제로의 범용 버전
  - 게임
    - Atari 게임
      - 다양한 게임에 대한 심층 강화학습의 성능 비교 : 49종 29종은 사람보다 높은 수준
  ...
  - 시스템 설정/제어
    - 딥마인드 데이터 센터의 온도 조절 비용을 강화학습을 통해 40% 절감함
- 강화학습의 응용 분야
  - 보드게임, 비디오 게임, 로봇 제어, 시스템 제어 등
    - 주어진 상황에서 최적의 제어 신호에 대한 **정확한 목표값을 모름**
    - 제어 동작 후 결과에 대한 **성공 여부의 평가는 가능**
  - 강화학습은 **제어 문제를 표현하고 해결하는 방법**으로 주로 사용
    - **주어진 조건이나 상황에서 어떤 동작을 취해야 할지를 결정**하는 문제
    - 시간에 따른 순차적인 개념이 존재 → 이전 상황이 현재 시점에서의 결정에 영향을 미침
- **강화학습(reinforcement learning)**
  - 출력값에 대한 교사 신호가 ‘보상’(reward) 형태로 제공
    - 목표 출력값 없음
    - 최종 목표를 지정하고 매 순간 학습 시스템이 취하는 행동의 결과에 따라 보상을 제공
  - 학습의 목적
    - 최종적으로 얻게 되는 누적 보상의 최대화
      - 목표 달성에 적절한 행동 → 양의 보상
      - 목표 달성에 부적절한 행동 → 음의 보상(벌점)
    - 보상을 통해 좋은/나쁜 행동을 배워 행동 방식을 긍정적 방향으로 “강화”한다.
- 강화학습의 계산 모델  
  ![image](https://user-images.githubusercontent.com/61646760/205443608-de2c1c6a-a600-4e71-a795-8e354c4e55c2.png)
  - 강화학습의 두 주체
    - 에이전트
      - 학습하는 대상(머신)
      - 그 역할은 누적 보상을 최대화하는 최적의 행동 결정
    - 환경
      - 처리하는 환경
  - 행동 `𝐴_𝑡`에 따라 기존 상태(`𝑆_𝑡`)가 다른 상태(`𝑆_{𝑡+1}`)로 바뀌고, 행동에 대한 보상(`𝑅_{𝑡+1}`)이 주어짐
    - 상태와 보상의 예
      - 미로 찾기  
        ![image](https://user-images.githubusercontent.com/61646760/205444160-eb5bd276-910d-45c3-b3da-54036fad7958.png)
- **마르코프 결정 프로세스(MDP, Markov Decision Process)**
  ...
  - **마르코프 성질(Markov property)**
    - “다음 행동은 현재의 상태(상황)에 의해서만 결정된다.”
      - 이전의 선택/결정들이 영향을 미치지 않음 → **현재 상태만 판단**하면 됨
    - 강화학습은 기본적으로 마르코프 성질을 만족한다는 가정하에 문제를 해결
      - 어떤 문제가 마르코프 성질을 만족하지 못하면 근사화시킨 뒤 문제를 해결
      - 즉, 주어진 문제를 강화학습으로 풀려면, 문제를 MDP로 모델링하는 것이 먼저 할 일
- 마르코프 결정 프로세스는 **<𝑆, 𝐴, 𝑃, 𝑅, 𝛾>의 튜플**이다.
  - `𝑆` : 가능한 상태의 유한집합
  - `𝐴` : 가능한 행동의 유한집합
  - `𝑃` : 상태 전이확률  
    ![image](https://user-images.githubusercontent.com/61646760/205446344-7651e4da-d71e-4c51-aa91-7e88bb0ec873.png)
    - `(s, a) → s'`로만 전이되면, 결정론적(deterministic)
    - 전이될 수 있는 상태가 여럿이면, 비결정론적(non-deterministic)
  - `𝑅` : 보상 함수  
    ![image](https://user-images.githubusercontent.com/61646760/205446421-1e9c2592-fc27-4305-bdc6-f7edab3c88ec.png)
  - `𝛾` : 보상을 계산할 때 사용되는 할인율(discount factor) (𝛾 ∈ [0,1])
- 미로 찾기 문제에 대한 MDP의 예  
  ![image](https://user-images.githubusercontent.com/61646760/205447426-6331bbde-8523-4cfb-8548-764b73608668.png)
  - 상태 집합 `𝑆`, 행동 집합 `𝐴`, 상태 전이확률 `𝑃`, 보상 집합 `𝑅`을 설정해 줌
- 정책과 가치함수
  - **정책(𝜋)**
    - 상태 `𝑆_𝑡`와 행동 `𝐴_𝑡`의 시퀀스를 결정하는 함수적 규칙  
      ![image](https://user-images.githubusercontent.com/61646760/205447610-e0a35875-6df5-4363-bd44-2721d052e7d8.png)
      - 정책의 예시  
        ![image](https://user-images.githubusercontent.com/61646760/205447642-87d9cbaf-a24c-446f-9f47-85c7a350255c.png)
  - ‘좋은 정책’에 대한 평가 기준이 필요
    - **수익=이익(return=gain)** `𝐺_𝑡`  
      ![image](https://user-images.githubusercontent.com/61646760/205447732-0493b8a4-49bc-49e2-a00a-2b7cb981a2e9.png)
      - **총할인 보상(total discount reward)**
        - 시점 𝑡에서부터 얻어지는 보상에 대해 할인율 𝛾을 곱해서 더한 값
          - 시점 𝑡에서 멀어질수록 `𝐺_𝑡`에 기여하는 정도가 낮으므로, 할인율 𝛾의 지수를 높여 주는 것
        - `총할인 보상 = 수익 = 이익 = 누적 보상`
  - 학습의 목적
    - 최종 누적 보상을 최대화하는 **최적 정책(optimal policy)** `𝜋∗: 𝑆 ↦ 𝐴`을 찾는 것
    - 어떻게 최적 정책을 찾을까?
      - 모든 정책(𝜋)들에 대해 평가 수행? 불가능!
      - 상태와 행동에 대한 가치 평가를 수행 → 가치함수로!
  - 가치함수
    - **상태 가치함수(state-value function)** `𝑣_𝜋(𝑠)`
      - 상태에 대해 가치 평가하는 함수
        - 그냥 value function이라고도 함
      - 상태 𝑠에서 시작해서 정책 𝜋에 따라 행동을 취하였을 때 얻을 수 있는 기대 보상  
        ![image](https://user-images.githubusercontent.com/61646760/205448282-cf35add3-7245-43cf-a22f-bfe631cf4a45.png)
    - **행동 가치함수(action-value function)** `𝑞_𝜋(𝑠,𝑎)`
      - 행동에 대해 가치 평가하는 함수
        - **Q-함수**라고도 함
      - 정책 𝜋에 따라 상태 𝑠에서 행동 𝑎를 선택하였을 때 얻을 수 있는 기대 보상  
        ![image](https://user-images.githubusercontent.com/61646760/205448295-a81e218f-4229-4579-aa6b-f2da2a877066.png)
- 최적의 정책과 가치함수
  - 최적 가치 함수
    - **최적 상태 가치함수(optimal state-value function)** `𝑣∗(𝑠)`
      - 가능한 모든 정책에 대해 최대값을 갖는 상태 가치함수  
        ![image](https://user-images.githubusercontent.com/61646760/205448695-de044f52-e0a1-4042-bee2-222cc4206ce6.png)
    - **최적 행동 가치함수(optimal action-value function)** `𝑞∗(𝑠,𝑎)`
      - 가능한 모든 정책에 대해 최대값을 갖는 행동 가치함수  
        ![image](https://user-images.githubusercontent.com/61646760/205448705-585dd545-1da8-44a8-b6e9-95b788943b03.png)
  - **최적 정책(optimal policy)**
    - 최적 행동 가치함수 `𝑞∗(𝑠,𝑎)`를 최대화하는 행동을 찾으면 됨  
      ![image](https://user-images.githubusercontent.com/61646760/205448737-0d0d153c-89d9-4951-b05a-91afe9c8a866.png)
      - 최적의 행동이면 1, 아니면 0
- MDP에서 학습의 어려움
...
- **Q-함수(Q-function)**
  - 행동 가치함수  
    ![image](https://user-images.githubusercontent.com/61646760/205446006-e3a16d28-bc9c-4794-bb94-1bc34c6b62af.png)
  - **Q-값(Q-value)**
    - 주어진 상태-행동 쌍의 기대 보상
    - 즉, Q-함수의 값
- **Q-학습(Q-learning)**
  - 최적 정책 `𝜋*`를 얻기 위해서 최적 Q-함수 `𝑄∗(𝑠,𝑎)`를 추정하는 방법
  - Q-학습 알고리즘  
    ![image](https://user-images.githubusercontent.com/61646760/205450412-2b524199-7e83-4ae5-8dbc-e311250c4f15.png)
  - 간단한 Q-학습의 예  
    ![image](https://user-images.githubusercontent.com/61646760/205450516-71cf64cd-9c94-420d-bf6d-943bb8281f19.png)
- **Q-테이블**
  - Q-함수의 값이 이산적이므로 테이블 형태로 표현 가능
    - 16가지 상태 × 4가지 행동 = 64개 값  
      ![image](https://user-images.githubusercontent.com/61646760/205450597-e9bab772-2075-452c-9c99-d4af3de3e66d.png)
      - Q-학습을 위해 64개 값을 0으로 초기화
      - 갱신 과정  
        ![image](https://user-images.githubusercontent.com/61646760/205450755-2dd9c195-fc61-47ad-a2fd-d8f2a63111af.png)  
        ![image](https://user-images.githubusercontent.com/61646760/205450799-b8178cd1-395a-43d5-a90e-27dbf44ab8c5.png)
        - 그런데 𝑅, 𝐿, 𝑈, 𝐷 모두 0이면 어떤 행동을 취해야 할까?
- 행동 선택을 위한 전략
  - 탐험 vs 탐사
    - **탐험(exploration)**
      - 탐색공간 전체를 골고루 찾음 → **랜덤하게 다른 행동**을 선택
    - **탐사(exploitation)**
      - 특정한 곳을 중심으로 주변을 집중적으로 찾음
      - **현재의 Q-함수 정보를 바탕으로 최적의 행동**을 선택
  - **엡실론 탐욕(ε-greedy)**
    - 혼합된 선택 전략  
      ![image](https://user-images.githubusercontent.com/61646760/205450946-875fa5e0-4f4b-465d-9c48-c317e757ff0c.png)
      - `ε`은 랜덤을 허용하는 정도
      - 시간이 지날수록(𝑡가 커질수록) 분모 값이 커져서 결과적으로 `ε`이 작아짐
        - 탐험은 점점 줄고, 탐사는 점점 늘어남  
- 어떤 경로가 더 좋은 것인가?  
  ![image](https://user-images.githubusercontent.com/61646760/205451390-f8f18fa9-32e8-4946-89b2-783becf0aad9.png)
  - **할인된 미래 보상(discounted future reward)**
    - 미래 보상에 할인율 𝛾을 적용  
      ![image](https://user-images.githubusercontent.com/61646760/205451324-133b1994-660c-4aec-bf94-994de68627b1.png)
    - 할인율을 적용한 Q-함수의 갱신 규칙  
      ![image](https://user-images.githubusercontent.com/61646760/205451387-718ab711-90c6-4339-b492-d130ca6f5b50.png)
  - 상태 11에서의 Q-값 (𝛾 = 0.9)  
    ![image](https://user-images.githubusercontent.com/61646760/205451482-dfbc4d52-38b8-4def-b8c6-b6dd514202bc.png)
    - 초록색 경로보다 빨간색 경로가 더 좋은 이유를 알 수 있음
- 비결정론적 환경
  - **결정론적 모델(deterministic model)**
    - 모델의 출력과 행동이 어떤 임의성(randomness)도 없이 초기 조건과 파라미터에 의해서만 전적으로 결정
    - 확률적 불확실성이 존재하지 않음
    - `예) 미로 문제`
  - **비결정론적 모델(non-deterministic model)**
    - 모델에 임의성(randomness)이 내재되어 있음
    - 동일한 초기 조건과 파라미터 집합일지라도 서로 다른 여러 출력을 생성
    - `예) 장기, 바둑, 체스 등`
  - 상태 전이와 `𝑄^(𝑠',𝑎')` → 비결정론적
    - 결정론적 버전의 Q-학습이 제대로 동작하지 않음  
      ![image](https://user-images.githubusercontent.com/61646760/205451889-7775a60c-dcb6-45bd-a836-185aa326b839.png)
    - 해결책
      - **신뢰 요소(belief factor)** `𝛼` 도입
  - 𝛼를 가진 Q-학습의 갱신 규칙  
    ![image](https://user-images.githubusercontent.com/61646760/205451925-6f933eff-9aea-44fc-9727-bb2226f1ab87.png)
    - 결정론적 버전의 Q-학습 부분은 𝛼만큼만 반영(𝛼만큼 신뢰)
- Q-테이블에서 Q-신경망으로
  - Q-테이블의 한계 : 실제 응용에서의 비효율성  
    ![image](https://user-images.githubusercontent.com/61646760/205451993-89f51794-4c0a-43c6-aa9a-0e7f5076fe00.png)
    - 이렇게 큰 테이블을 만든다고? 너무 비효율적
- **Q-신경망(Q-Network)**
  - 신경망을 이용하여 Q-함수를 표현하고 추정  
    ![image](https://user-images.githubusercontent.com/61646760/205452041-c1b5371f-fd80-417a-91d6-3c2cfa50dc6d.png)
  - Q-신경망의 학습
    - 시점 𝑡에서 학습 파라미터 𝜃에 대한 Q-신경망의 실제 출력  
      ![image](https://user-images.githubusercontent.com/61646760/205452222-fad9fb40-c3c1-4aa0-87cf-4c4c02e13d9b.png)
    - 목표 출력값  
      ![image](https://user-images.githubusercontent.com/61646760/205452277-fcc5303e-f886-426f-b455-a32c15c8b46e.png)
    - 목적함수  
      ![image](https://user-images.githubusercontent.com/61646760/205452291-50f421c7-36ff-4b2f-aca7-5516bbc52ea2.png)
...
- **DQN(Deep Q-Network)**
  - **경험 재현(experience replay)** : 데이터 간의 높은 상관관계에 따른 문제 해결
    - 재현을 통해 **학습 데이터의 시퀀스를 재구성**
    - 에이전트의 경험 (𝑠_𝑡, 𝑎_𝑡, 𝑟_𝑡, 𝑠_{𝑡+1})을 시간 간격 단위로 재현 메모리 𝐷에 저장한 후, 𝐷로부터 균등 무작위 추출을 통해 미니 배치를 구성하여 학습 진행
  - **목표망(target network)** : 시변적인 목표 출력값 문제 해결
    - 원래 Q-신경망과 같은 구조, 다른 파라미터 𝜃-를 가진 별도의 신경망  
      ![image](https://user-images.githubusercontent.com/61646760/205452683-6cc7a91c-7ea6-45cd-aa9e-2258e1cc79ee.png)
    - 목적함수  
      ![image](https://user-images.githubusercontent.com/61646760/205452702-de0bc801-6d13-4c9d-98bf-b60dd257db80.png)
      - 실제 출력 값은 원래 신경망으로, 목표 출력 값은 같은 구조지만 파라미터만 다른 신경망으로!
- DQN 학습 알고리즘  
  ![image](https://user-images.githubusercontent.com/61646760/205452798-051c093d-384f-4f02-85e9-c94a20b12de5.png)  
  ![image](https://user-images.githubusercontent.com/61646760/205452879-2cde3c97-0a1e-4fec-b0b3-f1bb921b6ee7.png)
