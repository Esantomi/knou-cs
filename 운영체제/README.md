...
- **FCFS 스케줄링(First-Come First-Served Scheduling)**
  ...
  - 준비 큐에 도착한 순서에 따라 디스패치  
    ...
    - A, B, C, D 순으로 도착했으니 A 먼저 디스패치
    - A가 끝나면 B, C, D 순으로 디스패치
    - 비선점 방식이므로 한번 디스패칭되면 해당 프로세스가 종료될 때까지 CPU 독점
  - 장점
    - 가장 간단한 스케줄링 기법
  - 단점
...
- FCFS 스케줄링의 예
  ...
  - 예시  
    ...
    - 준비 큐 A, B, C, D 순
    - 프로세스 A는 CPU 사이클이 7이므로 시각 7까지 실행
    - 시각 7에 B 디스패칭하고 시각 11에 B 종료
    - C는 시각 12, D는 시각 15에 종료
    - 스케줄링 평가 기준  
...
- SJF 스케줄링의 예
  ...
  - 예시  
    ...
    - 비선점 방식이므로 A가 시각 7만큼 실행됨
    - A가 실행되는 도중에 들어온 B, C는 각각 CPU 사이클이 4, 1이므로 실행 시간이 짧은 C가 먼저 실행됨
    - 결국 A, C, D, B 순서로 실행됨
  - 스케줄링 평가 기준  
...
- SRT 스케줄링의 예
  ...
  - 예시  
    ...
    - A는 CPU 사이클이 7이라 7만큼 시간이 필요하지만, 시각 2에 들어온 B는 4만큼 있으면 끝낼 수 있음
    - A를 2만큼 수행한 시점(완료까지 5 남음)에 준비 큐에 대기시키고 B 먼저 수행
    - B를 2만큼 수행한 시점(완료까지 2 남음)에 C가 들어옴
    - C는 시각 1이면 끝나므로, B도 준비 큐에 대기시키고 C 먼저 수행
    - A(5)보다 B(2)가 남은 시간이 더 짧으므로, 준비 큐에 B가 우선적으로 위치함
    - 시각 5에 프로세스 C가 종료되면서 D가 들어옴
    - 남은 시간은 각각 A(5), B(2), D(3)이므로 B 먼저 수행하고 그다음 순서는 D(3), A(5)
    - 시각 15에 전체 작업 종료
  - 스케줄링 평가 기준  
