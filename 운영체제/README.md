...
- 다양한 장치들
  ...
  - 나머지 장치들 : 프로세스 실행 시 데이터 입력이나 출력에 사용되는 입출력장치  
    ![image](https://github.com/Esantomi/knou/assets/61646760/76ef93ad-2ec9-4e8e-94d5-dfc32f862485)
- 입출력장치의 구분
  - 세 가지 범주
    - 전용장치
    - 공용장치
...
- **가상장치(virtual devices)**
  ...
  - 디스크 같은 공용장치를 이용
  - `예) 스풀링을 적용한 플로터 등`
    - 스풀링은 나중에 처리하거나 인쇄하기 위해 데이터를 저장하는 시스템 기능
### 장치의 구성
#### 논리적 구성
- **장치제어기(device controller)**
  ...
  - 운영체제가 보내는 출력을 장치에 맞게 변환  
    ![image](https://github.com/Esantomi/knou/assets/61646760/06a536ec-0879-4c1c-aab1-52bbbba28cdb)
    - 장치와 장치 제어기 모두 하드웨어의 범주에 속함
- **장치 드라이버(device driver)**
  - 응용 프로그램의 입출력 요청을 해당 장치에 맞도록 변환
  - 장치의 종류나 제조사에 따라 장치제어기가 이해하는 명령이나 명령의 종류가 다를 수 있기 때문
  - 보통 장치 제조사에서 해당 장치의 드라이버도 같이 제공  
    ![image](https://github.com/Esantomi/knou/assets/61646760/b0fa0154-94a9-410e-be6d-a4ee035f3b1f)
#### 물리적 구성
- CPU의 장치 사용법
  - 장치제어기의 레지스터를 이용
  - 장치의 상태를 확인하거나 장치에 명령  
    ![image](https://github.com/Esantomi/knou/assets/61646760/cc3c108f-1baa-4647-aa66-f5779b8920b6)
    - CPU가 메모리를 이용할 때는 메모리 주소를 사용, 장치를 이용할 때는 장치제어기의 레지스터 사용
- **메모리 사상 입출력(memory-mapped I/O)**
  - 메모리의 특정 영역을 장치제어기의 레지스터와 대응시켜 둠
  - 메모리를 읽고 쓰는 것으로 CPU가 장치를 사용  
    ![image](https://github.com/Esantomi/knou/assets/61646760/74b3dd3d-8f38-46d4-9efc-e28c130b759d) 
    - CPU가 장치제어기에서 직접 작업을 하는 게 아니라, 메모리의 특정 주소에 가서 작업함
### 입출력 처리 유형
- 프로세스가 진행하며 입출력이 발생하는 경우
- 세 가지 유형 : 아래로 내려갈수록 더 나은 방법이라고 보면 됨
  - 프로그램 방법
  - 인터럽트 방법
  - DMA 방법
- **프로그램 방법(programmed I/O)**
...
- **인터럽트 방법(interrupt driven I/O)**
  ...
  - 프로세스를 대기상태로 보내고 인터럽트가 발생할 때까지 CPU는 다른 프로세스를 처리할 수 있음  
    ![image](https://github.com/Esantomi/knou/assets/61646760/b18887d6-3f2f-477c-8230-3c6e4f280326)
  - 인터럽트 처리 과정  
    ![image](https://github.com/Esantomi/knou/assets/61646760/e482e547-4e0c-4be9-a7c7-c20566819020)
...
- **DMA 방법(DMA I/O)**
  ...
  - 인터럽트 발생 횟수를 단 한 번으로 줄여 CPU 효율 증대  
    ![image](https://github.com/Esantomi/knou/assets/61646760/828c84bf-112a-4d9b-8fc7-2bded7a5bb25)
  - DMA 처리 과정  
    ![image](https://github.com/Esantomi/knou/assets/61646760/ff5b8141-5f36-4f7f-b600-a8c9aaac9bc9)
...
- **버퍼링(buffering)**
  ...
  - 메모리를 일시적인 데이터 저장장소인 버퍼로 이용  
    ![image](https://github.com/Esantomi/knou/assets/61646760/6e8fdcd6-329b-4127-a7c7-eec74b4c5f90)
  - 단일 버퍼링
    - 저장과 처리를 동시에 할 수 없어 비효율적
  - 이중 버퍼링  
    ![image](https://github.com/Esantomi/knou/assets/61646760/edc2c156-605e-41c1-9642-3c63a96f6e72)
  - 순환 버퍼링  
    ![image](https://github.com/Esantomi/knou/assets/61646760/533f3314-38ef-4ae5-b814-bdfd90358e9f)
- **스풀링(spooling)**
  - 입출력 프로세스와 저속 입출력장치 사이의 데이터 전송을 자기 디스크와 같은 고속장치를 통하도록 하는 것  
    ![image](https://github.com/Esantomi/knou/assets/61646760/85e3e846-bbdc-45b5-8e18-983537d8d54f)
    - 버퍼링의 일종
...
- 디스크 접근 요구 처리 시간
  - `탐구시간(Seek time) + 회전지연시간(Latency time) + 전송시간`  
    ![image](https://github.com/Esantomi/knou/assets/61646760/928c2567-337e-4f7f-bca5-a209060855cf)
  - 스케줄링 형태
    - 탐구시간 최적화 : 핵심!!
    - 회전지연시간 최적화 : 거의 정해져 있음. SLTF 스케줄링
- **FCFS 스케줄링**
...
- **SLTF 스케줄링**
  ...
  - 동일 실린더 여러 섹터에 대한 접근 요구에 대해, 회전지연시간이 가장 짧은 것을 먼저 처리하는 방법
  - 높은 부하 상태에서 유용함
### 파일 관리
- **파일 관리자**
  - 파일을 생성, 삭제, 수정
...
- **파일 구조**
  ...
  - 접근 방식
    ...
    - 직접 파일
- **순차 파일**
  - 레코드가 **물리적 순서**에 따라 저장되어 있는 파일
  - 논리적 순서와 물리적 순서가 동일
  - 순차접근 저장장치에 많이 사용
- **인덱스된 순차 파일**
  - 각 레코드의 키를 기준으로 한 **논리적 순서**대로 레코드가 저장됨
  - 일부 주요 레코드의 **실제 주소가 저장된 인덱스를 구성하여 관리**하는 파일
...
- 분산 시스템의 네트워크 구성
  - 네트워크 연결 방법
    - **완전연결 네트워크**
      - 통신 비용, 가용성 측면에서 좋으나 구축 비용이 많이 듦
    - **부분연결 네트워크**
...
- **분산 파일 시스템(DFS, Distributed File System)**
  ...
  - 원격 파일에 대한 요청 처리
    - 원격 프로시저 호출(RPC, Remote Procedure Call)을 통해 구현
  - 캐시를 이용한 분산 파일 시스템
    - 전체 시스템의 성능 높임
### 분산 메모리
...
- **분산 공유 메모리(Distributed shared memory)**
  ...
  - 원격 메모리 클라이언트(아래에 원격 메모리 서버)가 가상 메모리와 연결
  - 장점
    - 노드 개수가 늘어나도 잘 확장됨
...
- **NUMA(Non-Uniform Memory Access)**
  - 각 프로세서가 로컬 메모리를 갖는 분산 공유 메모리  
    ![image](https://github.com/Esantomi/knou/assets/61646760/9cce127c-980a-4faa-ad67-5ec4b7a07814)
  - 저장 위치에 따라 속도 차이 발생
### 원격 프로시저 호출
