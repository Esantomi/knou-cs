...
- **합성함수(composition function)**
  - 𝑿, 𝒀, 𝒀′, 𝒁 : 집합, 𝒀′⊂𝒀
    - 두 함수 𝒇∶ 𝑿→𝒀′와 𝒈∶ 𝒀→𝒁에 대해 다음과 같이 정의되는 함수 𝒈∘𝒇∶ 𝑿→𝒁가 𝒇와 𝒈의 합성함수
      - ∀𝒙∈𝑿, **(𝒈∘𝒇)(𝒙) ≡ 𝒈(𝒇(𝒙))**  
        ![image](https://user-images.githubusercontent.com/61646760/230033104-d16de408-800a-4c2c-ad17-51dae89c07cc.png)
  - 정리
...
- 논리와 부울대수의 관련성  
  ...
  - OR(∨) ⟺ +
  - NOT(~) ⟺ ^{bar}  
    ![image](https://user-images.githubusercontent.com/61646760/230117182-c4f6fa7f-f330-4262-bc6e-fd9d547959dc.png)  
...
- 간소화 목적  
  ...
  - 두 논리 회로는 동일함
  - **하나의 진리표는 수없이 많은 부울함수로 표현**할 수 있고, **각각의 부울함수는 하나의 논리 회로에 1:1로 대응**함
  - 복잡한 부울함수(= 논리 회로)를 간소화함으로써 간단한 부울함수(= 논리 회로)로 표현할 수 있음
- 간소화 방법
  - 부울대수의 기본 공식들을 활용하면 됨
  - **항결합** : 두 개의 항을 결합하여 하나의 항으로 만드는 방법  
    ![image](https://user-images.githubusercontent.com/61646760/230124018-92edc33b-8872-4883-9ef3-33d91be53b46.png)
  - **문자 소거** : 중복된 문자를 제거하는 방법  
    ![image](https://user-images.githubusercontent.com/61646760/230124141-81c4d793-a150-4810-bfb2-0caa3a69a265.png)
  - **중복항 첨가** : 부울함수의 진릿값이 변하지 않도록 하면서 간소화를 위한 적절한 항을 첨가하는 방법  
    ![image](https://user-images.githubusercontent.com/61646760/230124274-2352666a-cac1-4b20-96b6-d8d3e9b32b87.png)

## 9강. 그래프 I
- [위키백과 - 그래프 이론 용어](https://ko.wikipedia.org/wiki/그래프_이론_용어)
### 9.1 기본사항
- **쾨니히스베르크의 다리 문제(Königsberg's bridge problem)**  
...
- 그래프의 주요 용어
  - 그래프 G는 '**꼭지점(vertex)**'들과 '**변(edge)**'들로 구성
    - 𝑮=(𝑽,𝑬)
      - 여기서 𝑽={𝒗|𝒗는 꼭지점}, 𝑬={𝒆|𝒆는 변}
    - 변은 두 꼭지점을 연결함
      - 변에 의해 '**발생(incident)**'되었다고 함
    - 연결된 두 꼭지점은 서로 '**인접(adjacent)**'한다고 함
  - **병렬 변(parallel edge)**
    - 두 꼭지점을 연결하는 변이 복수 개 있을 때
  - **루프(loop)**
    - 동일한 꼭지점을 연결하는 변
  - **고립된 꼭지점(isolated vertex)**
    - 어떠한 변도 연결되지 않은 꼭지점
  - **동형(isomorphic)**
    - 꼭지점과 변의 이름만 제외하고는 모두 동일한 그래프들을 서로 동형 이라고 함  
      ![image](https://user-images.githubusercontent.com/61646760/232199526-434948a3-0334-4d90-ba43-7a896ff8557b.png)
      - 네 번째 그래프를 제외한 세 그래프는 동형
- **방향 그래프(directed graph)**
  - 변이 방향을 가지고 있는 그래프
...
- **단순 그래프(simple graph)**
  - 루프와 병렬 변을 가지지 않는 무향 그래프  
    ![image](https://user-images.githubusercontent.com/61646760/232200035-a6021af4-0771-4df6-857b-2f246f19445b.png)
- 𝑮=(𝑽,𝑬), 𝑯=(𝑽′,𝑬′)가 그래프일 때 (아래는 𝑮)  
  ![image](https://user-images.githubusercontent.com/61646760/232200411-3ffbe584-d781-4c83-8eea-a7233b68dcc9.png)
  - **부분 그래프(subgraph)**
    - 𝑽′⊆𝑽, 𝑬′⊆𝑬 ⇒ 𝑯는 𝑮의 부분 그래프  
      ![image](https://user-images.githubusercontent.com/61646760/232200431-e9e43b30-7b91-4d7d-bea0-93d8792cece5.png)
  - **신장 부분 그래프(spanning subgraph)**
    - 𝑽′=𝑽, 𝑬′⊆𝑬 ⇒  𝑯는 𝑮의 신장 부분 그래프  
      ![image](https://user-images.githubusercontent.com/61646760/232200438-a7896059-0ae0-4e3e-92df-9d0984054ce8.png)
- 𝑮=(𝑽,𝑬), 𝒗∈𝑽일 때
  - 𝒗의 **차수(degree)** [𝐝𝐞𝐠(𝒗)]
    - 𝒗에 인접한 변의 개수
    - 방향 그래프에서
      - 𝒗의 **진입차수(in-degree)** : 𝒗로 들어오는 변의 수
      - 𝒗의 **진출차수(out-degree)** : 𝒗에서 나가는 변의 수
  - 𝑮의 **총차수(total degree)** [𝐝𝐞𝐠𝑮]
    - 𝑮에 속한 모든 꼭지점의 차수의 합
    - ∴ 𝐝𝐞𝐠(𝑮) = Σ_{𝒗∈𝑽}𝐝𝐞𝐠(𝒗)  
      ![image](https://user-images.githubusercontent.com/61646760/232200798-ac8c57e4-7156-408e-88f5-e1ad16ee48ae.png)
- **악수 보조정리(Handshaking Lemma)**
  - 임의의 그래프 𝑮의 총차수는 edge(𝑬) 개수의 2배  
    ![image](https://user-images.githubusercontent.com/61646760/232200922-64c75032-c3e8-4376-ae4e-2d005ec121fe.png)
    - 우측 그래프의 edge 개수는 7이므로 총차수는 14가 됨
    - 변은 반드시 두 개의 꼭지점을 발생(incident)시키기 때문
  - cf. 그래프에서 **차수가 홀수인 꼭지점의 수는 짝수**이다.
#### 그래프 탐색
- 워크, 트레일, 경로
  - **워크(walk)**
    - 𝒗_{𝟎}에서 𝒗_{𝒌}까지의 워크란, 𝒗_{𝟎}에서 시작하여 𝒗_{𝒌}에 도착하는 꼭지점과 변들을 순서대로 나열한 것  
      ![image](https://user-images.githubusercontent.com/61646760/232201506-7f37a302-276c-4121-8b1d-db3d4b8dc43f.png)
      - 𝒗_{𝟎} : 시작점
      - 𝒗_{𝒌} : 종점
      - 𝒗_{𝟏}, 𝒗_{𝟐} ⋯ 𝒗_{𝒌−𝟏}: 내부점
    - 𝒌 : W 의 길이(length)
    - 𝒗_{𝟎} = 𝒗_{𝒌}일 때 W는 '**닫혀 있다(closed)**'고 함
  - **트레일(trail)**
    - 워크 𝑾의 변들이 모두 다른 경우
    - 즉, **변이 중복되지 않는 워크**
  - **경로(path)**
    - 트레일 𝑾의 꼭지점이 모두 다른 경우
    - 즉, **꼭짓점이 중복되지 않는 트레일**
  - **path ⊂ trail ⊂ walk**  
    ![image](https://user-images.githubusercontent.com/61646760/232202348-8a0d7421-411a-42f8-9fba-2d4107f52e5a.png)
- 닫힌 워크, 닫힌 트레일, 사이클
  - 𝑮=(𝑽,𝑬), 𝑾=𝒗𝟎𝒆𝟏𝒗𝟏𝒆𝟐𝒗𝟐 ⋯ 𝒆𝒌𝒗𝒌 : walk일 때
    - **닫힌 워크**
      - 𝑾가 워크이고 𝒗_{𝟎} = 𝒗_{𝒌}인 경우
    - **닫힌 트레일**
      - 𝑾가 트레일이고 𝒗_{𝟎} = 𝒗_{𝒌}인 경우
    - **닫힌 경로 = 사이클(cycle)**
      - 𝑾가 경로이고 𝒗_{𝟎} = 𝒗_{𝒌}인 경우
    - **𝒌-사이클** : 길이가 𝒌인 사이클
- 정리  
  ![image](https://user-images.githubusercontent.com/61646760/232202724-ca53db38-bcc5-470f-b516-bb795b05f930.png)
- **연결 그래프(connected graph)**
  - 𝒖에서 𝒗로 가는 경로가 존재하면 𝒖와 𝒗는 서로 **연결**되어 있음
  - 𝑽의 꼭지점들은 서로 연결되고 다른 집합과 겹치지 않는 꼭지점들의 집합 𝑽𝟏,𝑽𝟐,⋯𝑽𝒏으로 나눌 수 있음
    - 𝑽𝟏,𝑽𝟐,⋯𝑽𝒏을 𝑮의 **연결성분**이라고 함
  - ∀𝒖, ∀𝒗∈𝑽, 𝒖에서 𝒗로 가는 경로(path)가 존재할 때
    - 𝑮는 연결 그래프
    - 𝑮는 **오직 하나의 연결 성분으로 구성**됨
  - `예) 다음 각 그래프에 대해서 연결 성분을 찾고 연결 그래프인지 판별하시오.`  
    ![image](https://user-images.githubusercontent.com/61646760/232203060-bc125885-aabf-4eaf-a68f-081ac72d7215.png)
### 9.2 그래프의 종류
- **완전 그래프(complete graph)**
  - ∀𝒖, ∀𝒗∈𝑽, ∃𝒆 = (𝒖,𝒗)∈𝑬
    - 𝑮는 완전 그래프
    - 즉, 완전 그래프는 **임의의 두 꼭지점을 연결하는 변이 항상 존재**하는 그래프
  - 𝑲_{𝒏} ∶ |𝑽|=𝒏인 완전 그래프
    - cf. 완전 그래프 𝑲_{𝒏}은 **𝒏(𝒏−𝟏)/𝟐**개의 변을 가지며 모든 꼭지점의 차수는 **𝒏−𝟏**임
    - 완전그래프 𝑲_{𝒏}은 **(n-1)-정규 그래프**
  - 완전 그래프 𝑲𝟏, 𝑲𝟐, 𝑲𝟑, 𝑲𝟒, 𝑲𝟓, 𝑲𝟔는 다음과 같음  
    ![image](https://user-images.githubusercontent.com/61646760/232203268-17800ef0-b6ab-4496-a4ba-26fcabd64109.png)
    - 𝑲𝟏 : 𝒆 개수 1*0/2 = 0, 각 𝒗 차수 1-1 = 0
    - 𝑲𝟐 : 𝒆 개수 2*1/2 = 1, 각 𝒗 차수 2-1 = 1
    - 𝑲𝟑 : 𝒆 개수 3*2/2 = 3, 각 𝒗 차수 3-1 = 2
- **이분 그래프(bipartite graph)**
  - 𝑮=(𝑽,𝑬)에서 𝑽는 **연결성분 𝑽_{𝟏}과 𝑽_{𝟐}로 분할**되어 있고 **모든 변들이 𝑽_{𝟏}의 꼭지점과 𝑽_{𝟐}의 꼭지점을 인접시키는 경우
    - 1. 𝑽𝟏∪𝑽𝟐 = 𝑽, 𝑽𝟏∩𝑽𝟐 = ∅
    - ∀𝒆 = (𝒗𝟏,𝒗𝟐)∈𝑬, 𝒗𝟏∈𝑽𝟏, 𝒗𝟐∈𝑽𝟐  
      ![image](https://user-images.githubusercontent.com/61646760/232205304-8d31d05a-bef7-48ac-9f1b-d898855be47e.png)
      - 첫 번째 그래프는 위의 2개 vertex와 아래 3개 vertex를 각각 연결성분 𝑽𝟏, 𝑽𝟐로 보면 됨
      - 두 번째 그래프는 가운데 2개 vertex를 연결성분 𝑽𝟏로 보고, 나머지 점들을 𝑽𝟐로 보면 됨(입체적으로 생각할 것)
- **완전 이분 그래프(complete bipartite graph)**
  - 𝑮=(𝑽,𝑬)에서 ∀𝒗𝟏∈𝑽𝟏, ∀𝒗𝟐∈𝑽𝟐, ∃𝒆 = (𝒗𝟏,𝒗𝟐)∈𝑬인 경우
    - 즉, 꼭짓점의 집합이 서로 겹치지 않는 두 집합 𝑽𝟏와 𝑽𝟐의 합집합이고 𝑽𝟏의 모든 꼭짓점이 𝑽𝟐의 각각의 꼭짓점과 하나의 변으로 연결되어 있는 이분 그래프
    - 이분 그래프와 비교해 보면, 𝒗𝟏∈𝑽𝟏, 𝒗𝟐∈𝑽𝟐가 아니라 ∀𝒗𝟏∈𝑽𝟏, ∀𝒗𝟐∈𝑽𝟐여야 함
  - 𝑲_{𝒎,𝒏} ∶ |𝑽𝟏|=𝒎, |𝑽𝟐|=𝒏인 완전 이분 그래프  
    ![image](https://user-images.githubusercontent.com/61646760/232205581-6ceb9a9b-270f-4ac8-a4e1-6c4a69a20bb8.png)
- **정규 그래프(regular graph)**
  - 𝑮=(𝑽,𝑬)에서 𝑮의 **모든 꼭지점들이 동일한 수의 인접한 꼭짓점을 갖는 경우**
  - 정규 그래프 𝑮는 모든 꼭지점이 동일한 차수를 가짐
    - 즉, ∀𝒗∈𝑽, 𝐝𝐞𝐠(𝒗)=𝒌
    - 따라서 정규 그래프 𝑮를 '**𝒌-정규 그래프(k-regular graph)**'라고 함  
      ![image](https://user-images.githubusercontent.com/61646760/232205775-7d7a9786-2615-470a-8a72-8fb20d7a40b6.png)
  - 완전그래프 𝑲_{𝒏}은 **(n-1)-정규 그래프**
### 9.3 그래프의 표현
- **발생행렬(incidence matrix)**
  - **꼭짓점을 행**으로, **변을 열**로 하여 변과 꼭지점 사이의 발생관계를 표현한 행렬
    - 발생 행렬 𝑴_{𝑰}=(𝒂_{𝒊𝒋})
  - |𝑽|×|𝑬| 크기의 부울행렬
    - 𝒂_{𝒊𝒋} = 1 (𝒗_{𝒊}가 𝒆_{𝒋}에 의해 발생되는 경우)
    - 𝒂_{𝒊𝒋} = 0 (그 밖의 경우)  
      ![image](https://user-images.githubusercontent.com/61646760/232205988-1505ecaf-accc-4f6a-9849-74384521570d.png)
- **인접행렬(adjacency matrix)**
  - 꼭짓점을 행과 열로 하여 꼭짓점과 꼭짓점 사이의 인접관계를 표현한 행렬
    - 인접 행렬 𝑴_{𝑨}=(𝒂_{𝒊𝒋})
  - |𝑽|×|𝑽| 크기의 행렬
    - 𝒂_{𝒊𝒋} = 𝒗_{𝒊}에서 𝒗_{𝒋}로의 연결 개수  
      ![image](https://user-images.githubusercontent.com/61646760/232206156-3f2a5ec2-f3a7-42b6-a5d0-25fa2d61399b.png)
